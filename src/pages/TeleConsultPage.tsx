...
export const TeleConsultPage: React.FC = () => {
  ...
  // Form States for Advance Booking (Flexible Time Window)
  const [selectedDate, setSelectedDate] = useState<'Today' | 'Tomorrow' | 'Day After'>('Today');
  const [selectedTimeWindow, setSelectedTimeWindow] = useState<string>('10:00 AM – 12:00 PM');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-xl w-full border border-slate-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleConfirmBooking} className="p-6 space-y-4 overflow-y-auto text-xs">
              ...
              {/* Dynamic 2-Hour Flexible Arrival Window Selection */}
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block font-bold text-slate-700 uppercase tracking-wider text-[11px]">
                    2. Choose Arrival Time Window *
                  </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {STANDARD_TIME_WINDOWS.map(win => {
                    const isSelected = selectedTimeWindow === win.label;
                    return (
                      <button
                        key={win.id}
                        type="button"
                        onClick={() => setSelectedTimeWindow(win.label)}
                      ...
                      >
                        <div>
                          <span className="text-xs font-bold text-slate-900 block font-mono">{win.label}</span>
                          <span className="text-[10px] text-slate-500">{win.period} Arrival Window</span>
                        </div>
                        {isSelected && (
                ...
                {/* Sequential Token Assignment Live Preview */}
                {(() => {
                  const previewToken = bookingDoctor 
                    ? generateNextToken(appointments, bookingDoctor.doctor.id, selectedTimeWindow)
                    : { tokenNumber: 'A-01', tokenSequence: 1 };
                  
                  return (
                    <div className="mt-3 p-3.5 bg-gradient-to-r from-teal-50 to-emerald-50 rounded-xl border border-teal-200 text-xs space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-extrabold text-teal-900 flex items-center gap-1.5">
                          <Sparkles className="w-3.5 h-3.5 text-teal-600" />
                          Sequential Token Assigned:
                        </span>
                        <span className="px-2.5 py-1 rounded-lg bg-teal-700 text-white font-mono font-black text-xs shadow-xs">
                          #{previewToken.tokenNumber}
                        </span>
                      </div>
                      <p className="text-[11px] text-teal-800 leading-relaxed">
                        Instead of waiting for an exact minute slot, you are assigned <strong>Token #{previewToken.tokenNumber}</strong> for the <strong>{selectedTimeWindow}</strong> window. You will receive dynamic queue updates and an estimated call time based on the doctor’s live consultation pace.
                      </p>
                    </div>
                  );
                })()}
          ...
          <div className="bg-white rounded-3xl max-w-md w-full border border-slate-200 shadow-2xl p-6 text-center space-y-4">
            ...
            <div>
              <h3 className="font-extrabold text-xl text-slate-900 font-heading">
                Queue Token Reserved!
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Your consultation token is confirmed in the e-Sanjeevani virtual queue.
              </p>
            </div>

            {/* Pass details */}
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-left text-xs space-y-2.5 font-medium">
              <div className="flex items-center justify-between border-b border-teal-200 pb-2.5 bg-teal-50/80 -mx-4 -mt-4 p-3 rounded-t-2xl">
                <div>
                  <span className="text-[10px] font-bold text-teal-800 uppercase tracking-wider block">OPD Queue Token</span>
                  <span className="text-xs font-semibold text-slate-600">Sequential Turn</span>
                </div>
                <span className="px-3 py-1 bg-teal-700 text-white rounded-lg font-mono font-black text-sm shadow-xs">
                  #{bookingSuccessModal.tokenNumber}
                </span>
              </div>
              ...
              <div className="flex justify-between border-b border-slate-200/80 pb-1.5">
                ...
                <span className="font-bold text-slate-900 font-mono">{bookingSuccessModal.timeWindow || bookingSuccessModal.timeSlot}</span>
              </div>
              <div className="flex justify-between border-b border-slate-200/80 pb-1.5">
                <span className="text-slate-500">Estimated Consultation:</span>
                <span className="font-bold text-emerald-700">{bookingSuccessModal.estimatedConsultationTime || 'Calculating...'}</span>
              </div>
              <div className="flex justify-between border-b border-slate-200/80 pb-1.5">
                <span className="text-slate-500">Queue Position:</span>
                <span className="font-bold text-teal-700">
                  {bookingSuccessModal.patientsAhead === 0 ? 'You are next in line' : `${bookingSuccessModal.patientsAhead} patients ahead`}
                </span>
              </div>
