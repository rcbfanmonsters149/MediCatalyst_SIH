  ...
  const [appointments, setAppointments] = useState<TeleAppointment[]>(() => {
    ...
    if (saved) {
      try {
        ...
        // Clean out any legacy completed consultation records from localStorage
        const cleaned = parsed.filter(a => a.id !== 'appt-call-completed-01' && a.id !== 'appt-call-completed-02');
        if (cleaned.length !== parsed.length) {
          localStorage.setItem('medcatalyst_tele_appointments', JSON.stringify(cleaned));
        }
        return cleaned;
      } catch (e) {
        console.error(e);
